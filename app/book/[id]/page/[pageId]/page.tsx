...
import { ChevronLeft, Save, Sparkles, Undo, Redo, Check, X } from "lucide-react";
import { Roboto_Serif, Sarabun } from "next/font/google";
import { useBooks } from "../../../../context/BookContext"; 
import { toast, Switch } from "@heroui/react";
  ...
  const handleSave = () => {
    ...
    setTimeout(() => {
      toast.success("Saved successful!", {
        ...
        actionProps: { children: "Got it!", className: "bg-success text-success-foreground font-medium px-4 py-2 rounded-full", onPress: () => toast.clear() }
      });
    }, 100);
  };

  // --- Logic: History (Undo/Redo/Manual Type) ---
  ...
  return (
    <div className="h-screen bg-slate-50 flex flex-col font-sans overflow-hidden">
      ...
      <main className="flex-1 flex flex-col md:flex-row gap-4 px-6 pb-6 overflow-hidden">
        ...
        {/* --- ฝั่งซ้าย: Original --- */}
        <div className="flex-1 bg-white border border-slate-200 rounded-[24px] p-8 shadow-sm flex flex-col transition-all">
          ...
          <div className="flex justify-between items-center mb-6 shrink-0">
            ...
            <div className="flex items-center gap-3">
              ...
              {/* 🚀 สวิตช์คลีนๆ พี่เช็กแล้ว ไม่มี color ไม่มี .target.checked รับรองเรนเดอร์ผ่านฉลุย! */}
              <Switch
                isSelected={isAiMode}
                onChange={setIsAiMode}
                size="md"
                aria-label="Toggle AI Translation Mode"
              />
            </div>
          ...
          {isAiMode && (
            <div className="shrink-0 mt-4 border-t border-slate-100 pt-6 flex flex-col gap-3 animate-in fade-in slide-in-from-bottom-4 duration-300">
              <textarea
                ...
                className={`w-full h-24 resize-none outline-none text-slate-700 text-base p-4 bg-slate-50 rounded-2xl border border-slate-200 focus:border-blue-300 focus:ring-4 focus:ring-blue-100/50 transition-all ${robotoSerif.className}`}
              />
        ...
        {/* --- ฝั่งขวา: Translation --- */}
        <div className={`flex-1 rounded-[24px] p-8 shadow-sm flex flex-col transition-all duration-500 ease-in-out ${
          isAiMode 
          ? "bg-blue-500/[.03] border-2 border-blue-400" 
          ...
          {isAiMode && aiPreviewText && (
            <div className="shrink-0 mt-4 p-5 bg-white border border-blue-200 rounded-2xl shadow-lg shadow-blue-100/50 flex flex-col gap-4 animate-in fade-in zoom-in-95 duration-300">
              ...
              <p className={`text-slate-800 text-lg leading-relaxed ${sarabun.className}`}>
                {aiPreviewText}
              </p>
              <div className="flex justify-end gap-2 mt-2 border-t border-slate-50 pt-4">
                <button 
                  onClick={handleCancelTranslation} 
                  className="flex items-center gap-1.5 px-4 py-2 rounded-full text-sm font-bold text-slate-500 hover:bg-slate-100 hover:text-slate-700 transition-colors"
                >
                  <X className="w-4 h-4" /> Cancel
                </button>
                <button 
                  onClick={handleAcceptTranslation} 
                  className="flex items-center gap-1.5 px-5 py-2 rounded-full text-sm font-bold text-white bg-blue-600 hover:bg-blue-700 shadow-md shadow-blue-200 transition-all active:scale-95"
                >
          ...
          {!aiPreviewText && (
            <div className="flex justify-end gap-3 shrink-0 mt-4">
              <button onClick={handleUndo} disabled={historyIndex <= 0} className={`flex items-center gap-2 px-5 py-2.5 rounded-full text-[14px] font-bold transition-all ${historyIndex <= 0 ? "bg-slate-200/50 text-slate-400 cursor-not-allowed border border-transparent" : "bg-white text-slate-600 hover:bg-slate-100 border border-slate-200 shadow-sm"}`}>
                <Undo className="w-4 h-4" /> Undo
              </button>
              <button onClick={handleRedo} disabled={historyIndex >= history.length - 1} className={`flex items-center gap-2 px-5 py-2.5 rounded-full text-[14px] font-bold transition-all ${historyIndex >= history.length - 1 ? "bg-slate-200/50 text-slate-400 cursor-not-allowed border border-transparent" : "bg-white text-slate-600 hover:bg-slate-100 border border-slate-200 shadow-sm"}`}>
