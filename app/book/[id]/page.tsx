  ...
  return (
    <div className="min-h-screen bg-[var(--background)] text-[var(--foreground)] font-sans pb-20">
      <div className="pt-6 px-6 max-w-7xl mx-auto">
        <header className="flex items-center justify-between px-6 py-3 bg-[var(--surface)] border border-[var(--border)] rounded-[var(--radius)] shadow-sm">
          ...
          <nav className="hidden md:flex items-center gap-1">
            <Link href="/" className="flex items-center gap-2 px-5 py-2.5 text-sm font-bold text-[var(--muted)] hover:bg-[var(--surface)] rounded-full transition-all"><LayoutGrid className="w-4 h-4" /> Overview</Link>
            <Link href="/" className="flex items-center gap-2 px-5 py-2.5 text-sm font-bold text-[var(--accent-foreground)] bg-[var(--accent)] rounded-full transition-all"><Book className="w-4 h-4" /> Library</Link>
            <button className="flex items-center gap-2 px-5 py-2.5 text-sm font-bold text-[var(--muted)] hover:bg-[var(--surface)] rounded-full transition-all"><Bookmark className="w-4 h-4" /> Bookmark</button>
          </nav>
          <div className="flex items-center gap-3">
            <div className="relative hidden sm:flex items-center w-[260px] h-10 text-[var(--muted)]">
              ...
              <input type="text" placeholder="Search books" className="w-full h-full pl-10 pr-4 text-sm bg-transparent border border-[var(--border)] rounded-full outline-none focus:border-[var(--focus)] transition-all text-[var(--foreground)] placeholder-[var(--muted)]" />
            </div>
            <button className="w-10 h-10 flex items-center justify-center border border-[var(--border)] rounded-full bg-transparent hover:bg-[var(--surface-secondary)] transition-colors text-[var(--foreground)]"><Bell className="w-4 h-4" /></button>
            <button className="w-10 h-10 flex items-center justify-center border border-[var(--border)] rounded-full bg-transparent hover:bg-[var(--surface-secondary)] transition-colors text-[var(--foreground)]"><Settings className="w-4 h-4" /></button>
      ...
      <main className="max-w-[1000px] mx-auto px-6 pt-10">
        ...
        <div className="bg-[var(--surface)] rounded-[var(--radius)] shadow-sm border border-[var(--border)] p-6  mb-12 flex flex-col md:flex-row gap-8 relative">
          <div className="absolute top-6 right-6 flex items-center gap-1">
            <button onClick={handleOpenEditModal} className="flex items-center gap-1.5 px-3 py-1.5 text-sm font-normal text-[var(--foreground)] hover:bg-[var(--surface-secondary)] rounded-full transition-colors border border-[var(--border)] bg-transparent"><Edit className="w-4 h-4" /> Edit book</button>
            <button onClick={() => setIsDeleteBookModalOpen(true)} className="flex items-center gap-1.5 px-3 py-1.5 text-sm font-normal text-[var(--foreground)] hover:bg-[var(--danger)]/10 hover:text-[var(--danger)] hover:border-[var(--danger)]/30 rounded-full transition-colors border border-[var(--border)] bg-transparent"><Trash2 className="w-4 h-4" /> Delete</button>
          </div>
        ...
        <div>
          {(pages.length > 0 || isAddingPage) && (
            <div className="flex justify-between items-center mb-6">
              ...
              <Button onPress={() => setIsAddingPage(true)} className="rounded-full bg-[var(--accent)] text-[var(--accent-foreground)] font-bold px-6"><Plus className="w-4 h-4 stroke-[2.5]" /> Add page</Button>
            </div>
          )}
          ...
          {pages.length === 0 && !isAddingPage ? (
            <div className="bg-[var(--surface)] border border-[var(--border)] shadow-sm rounded-[var(--radius)] flex flex-col items-center justify-center py-20 text-center ">
              ...
              <Button onPress={() => setIsAddingPage(true)} className="rounded-full bg-[var(--accent)] text-[var(--accent-foreground)] font-bold px-6"><Plus className="w-4 h-4 stroke-2" /> Add page</Button>
            </div>
          ) : (
              ...
              <div className="flex flex-col">
                {pages.map((page, index) => (
                  <div key={page.id} className="grid grid-cols-[80px_1fr_160px] gap-4 px-6 py-3 border-b border-[var(--border)] group items-center">
                    <div className="text-center font-bold text-[var(--muted)]"><div className="w-6 h-6 rounded-md bg-[var(--background)] border border-[var(--border)] flex items-center justify-center mx-auto text-xs">{index + 1}</div></div>

                    {editingPageId === page.id ? (
                      <div>
                        <input autoFocus type="text" value={editPageTitle} onChange={(e) => setEditPageTitle(e.target.value)} onKeyDown={handleKeyDownEdit} onBlur={handleSaveEdit} className="w-full px-4 py-2 bg-[var(--surface-secondary)] border border-[var(--border)] rounded-xl outline-none focus:border-[var(--focus)] focus:ring-1 focus:ring-transparent text-sm font-bold text-[var(--foreground)] transition-all"/>
                      </div>
                    ) : (
                    ...
                    <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      {editingPageId !== page.id && (
                        <>
                          <Button isIconOnly variant="outline" onPress={() => router.push(`/book/${book.id}/page/${page.id}`)} className="rounded-full border-[var(--border)] text-[var(--foreground)] hover:bg-[var(--surface-secondary)]"><Eye className="w-4 h-4" /></Button>
                          <Button isIconOnly variant="outline" onPress={() => startEditing(page)} className="rounded-full border-[var(--border)] text-[var(--foreground)] hover:bg-[var(--surface-secondary)]"><Edit className="w-4 h-4" /></Button>
                        </>
                      )}
                      <Button isIconOnly variant="outline" onPress={() => setPageToDelete(page.id)} className="rounded-full border-[var(--border)] text-[var(--foreground)] hover:bg-[var(--danger)]/15 hover:border-[var(--danger)]/30 hover:!text-[var(--danger)]"><Trash2 className="w-4 h-4" /></Button>
                    </div>
                  </div>
                ))}
                ...
                {isAddingPage && (
                  <div className="grid grid-cols-[80px_1fr_160px] gap-4 px-6 py-3 border-b border-[var(--focus)] bg-[var(--surface)] items-center">
                    <div className="text-center font-bold text-[var(--muted)]"><div className="w-6 h-6 rounded-md bg-[var(--background)] border border-[var(--border)] flex items-center justify-center mx-auto text-xs">{pages.length + 1}</div></div>
                    <div>
                      <input autoFocus type="text" placeholder="Enter the content" value={newPageTitle} onChange={(e) => setNewPageTitle(e.target.value)} onKeyDown={handleKeyDownNew} onBlur={handleSaveNewPage} className="w-full px-4 py-2 bg-[var(--surface-secondary)] border border-[var(--focus)] rounded-xl outline-none focus:border-[var(--focus)] focus:ring-1 focus:ring-transparent text-sm font-bold text-[var(--foreground)] transition-all placeholder:font-medium placeholder-[var(--muted)]"/>
                    </div>
                    <div className="flex items-center justify-end gap-2">
                      <div className="w-10 h-10 flex items-center justify-center bg-transparent border border-[var(--border)] text-[var(--muted)] rounded-full"><Eye className="w-4 h-4" /></div><div className="w-10 h-10 flex items-center justify-center bg-transparent border border-[var(--border)] text-[var(--muted)] rounded-full"><Edit className="w-4 h-4" /></div><div className="w-10 h-10 flex items-center justify-center bg-[var(--danger)]/10 border border-[var(--danger)]/20 text-[var(--danger)] rounded-full"><Trash2 className="w-4 h-4" /></div>
                    </div>
                  </div>
                )}
              </div>
      ...
      {isEditBookModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[var(--foreground)]/40 backdrop-blur-sm p-4">
          <div className="bg-[var(--surface)] border border-[var(--border)] rounded-[var(--radius)] shadow-xl w-full max-w-3xl  p-8 animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="flex flex-col gap-6">
              <div className="flex flex-col md:flex-row gap-8 items-stretch">
                ...
                <div className="w-full md:w-2/3 flex flex-col gap-4">
                  <TextField className="flex flex-col gap-1.5" isRequired>
                    ...
                    <Input type="text" value={editFormData.title} onChange={e => setEditFormData({...editFormData, title: e.target.value})} className="px-4 py-3 bg-[var(--surface)] rounded-xl outline-none focus:ring-2 focus:ring-transparent focus:border-[var(--focus)] font-medium border border-[var(--border)] transition-all text-[var(--foreground)]" />
                  </TextField>
                  <div className="grid grid-cols-2 gap-4">
                    <TextField className="flex flex-col gap-1.5" isRequired>
                      <Label className="text-sm font-semibold text-[var(--foreground)]">Author name</Label>
                      <Input type="text" value={editFormData.author} onChange={e => setEditFormData({...editFormData, author: e.target.value})} className="px-4 py-3 bg-[var(--surface)] rounded-xl outline-none focus:ring-2 focus:ring-transparent focus:border-[var(--focus)] font-medium border border-[var(--border)] transition-all text-[var(--foreground)]" />
                    </TextField>
                    <TextField className="flex flex-col gap-1.5" isRequired>
                      <Label className="text-sm font-semibold text-[var(--foreground)]">Total page</Label>
                      <Input type="number" value={editFormData.totalPages.toString()} onChange={e => setEditFormData({...editFormData, totalPages: Number(e.target.value)})} className="px-4 py-3 bg-[var(--surface)] rounded-xl outline-none focus:ring-2 focus:ring-transparent focus:border-[var(--focus)] font-medium border border-[var(--border)] transition-all text-[var(--foreground)]" />
                    </TextField>
                  </div>
                  <TextField className="flex flex-col gap-1.5">
                    <Label className="text-sm font-semibold text-[var(--foreground)]">Plot info</Label>
                    <TextArea rows={3} value={editFormData.plot} onChange={e => setEditFormData({...editFormData, plot: e.target.value})} className="px-4 py-3 bg-[var(--surface)] rounded-xl outline-none focus:ring-2 focus:ring-transparent focus:border-[var(--focus)] font-medium border border-[var(--border)] resize-none transition-all text-[var(--foreground)]" />
                  </TextField>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  >
                    <Select.Trigger className="w-full min-h-[48px] px-4 rounded-xl bg-[var(--surface)] hover:bg-[var(--surface-secondary)]  border border-[var(--border)] transition-all flex items-center justify-between">
                    ...
                    <Select.Popover placement="bottom" shouldFlip={false} className="w-(--trigger-width) bg-[var(--surface-secondary)] text-[var(--foreground)] rounded-xl  border border-[var(--border)] max-h-[140px] overflow-y-auto overflow-x-hidden">
                      <ListBox items={languages} aria-label="Original Language Options" className="p-1 flex flex-col gap-1 outline-none">
                        {(lang) => (
                          <ListBox.Item key={lang.value} id={lang.value} textValue={lang.label} className="w-full px-3 py-2 rounded-lg text-[var(--foreground)] font-medium cursor-pointer hover:bg-[var(--accent)] hover:text-[var(--accent-foreground)] transition-colors outline-none">
                            {lang.label}
                          </ListBox.Item>
                        )}
                      </ListBox>
            ...
            <div className="flex justify-end gap-3 mt-8 pt-6 border-t border-[var(--border)]"><button onClick={() => setIsEditBookModalOpen(false)} className="px-8 py-2.5 rounded-full font-bold text-[var(--foreground)] hover:bg-[var(--surface)] bg-transparent border border-[var(--border)]  transition-all">Cancel</button><button onClick={handleUpdateBook} className="px-10 py-2.5 rounded-full font-bold bg-[var(--accent)] text-[var(--accent-foreground)] hover:opacity-90  transition-all">Update</button></div>
          </div>
        </div>
      )}
      ...
      {isDeleteBookModalOpen && (
         <div className="fixed inset-0 z-60 flex items-center justify-center bg-[var(--foreground)]/40 backdrop-blur-sm p-4">
           <div className="bg-[var(--surface)] border border-[var(--border)] shadow-xl rounded-[var(--radius)] w-full max-w-[460px]  p-8 animate-in fade-in zoom-in-95 duration-200">
             <div className="flex justify-between items-start mb-4"><div className="w-12 h-12 rounded-full bg-[var(--danger)]/15 flex items-center justify-center text-[var(--danger)]"><Trash2 className="w-6 h-6" /></div><button onClick={() => setIsDeleteBookModalOpen(false)} className="p-2 text-[var(--muted)] hover:bg-[var(--surface-secondary)] rounded-full bg-[var(--surface)] transition-colors"><X className="w-5 h-5" /></button></div>
             ...
             <div className="flex justify-end gap-3"><button onClick={() => setIsDeleteBookModalOpen(false)} className="px-8 py-2.5 rounded-full font-bold text-[var(--foreground)] bg-transparent border border-[var(--border)] hover:bg-[var(--surface-secondary)]  transition-all">Cancel</button><button onClick={handleConfirmDeleteBook} className="px-10 py-2.5 rounded-full font-bold text-[var(--danger)] bg-[var(--danger)]/15 hover:bg-[var(--danger)]/25 transition-all">Delete</button></div>
           </div>
         </div>
      )}
      ...
      {pageToDelete !== null && (
         <div className="fixed inset-0 z-60 flex items-center justify-center bg-[var(--foreground)]/40 backdrop-blur-sm p-4">
           <div className="bg-[var(--surface)] border border-[var(--border)] shadow-xl rounded-[var(--radius)] w-full max-w-[460px]  p-8 animate-in fade-in zoom-in-95 duration-200">
             <div className="flex justify-between items-start mb-4"><div className="w-12 h-12 rounded-full bg-[var(--danger)]/15 flex items-center justify-center text-[var(--danger)]"><Trash2 className="w-6 h-6" /></div><button onClick={() => setPageToDelete(null)} className="p-2 text-[var(--muted)] hover:bg-[var(--surface-secondary)] rounded-full bg-[var(--surface)] transition-colors"><X className="w-5 h-5" /></button></div>
             ...
             <div className="flex justify-end gap-3"><button onClick={() => setPageToDelete(null)} className="px-8 py-2.5 rounded-full font-bold text-[var(--foreground)] bg-transparent border border-[var(--border)] hover:bg-[var(--surface-secondary)]  transition-all">Cancel</button><button onClick={handleConfirmDeletePage} className="px-10 py-2.5 rounded-full font-bold text-[var(--danger)] bg-[var(--danger)]/15 hover:bg-[var(--danger)]/25 transition-all">Delete</button></div>
           </div>
         </div>
      )}
    </div>
